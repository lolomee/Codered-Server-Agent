...
with open(conf_path, "r") as f:
    content = f.read()
# Replace any existing <address> tag
new_content = re.sub(r"<address>[^<]*</address>", "<address>" + manager_ip + "</address>", content)
# If no address tag found, insert after <client> tag
...
if python3 -c "import xml.etree.ElementTree as ET; ET.parse('$OSSEC_CONF')" 2>/dev/null; then
  ...
  warn "ossec.conf XML check failed — restoring backup and trying raw sed fallback..."
...
grep -q "$MANAGER_IP" "$OSSEC_CONF" && ok "Manager IP confirmed in config." || warn "Could not confirm IP — check $OSSEC_CONF manually."
